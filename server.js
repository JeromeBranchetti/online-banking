...
function createRefreshToken(payload) {
  return jwt.sign(payload, SECRET_KEY_REFRESHTOKEN, {
    expiresIn: refreshTokenExpiresIn,
  });
}

function verifyToken(token) {
  return jwt.verify(token, SECRET_KEY_TOKEN, (err, decode) =>
    decode !== undefined ? null : err
  );
}

function verifyTokenForVerificationEndPoint(token) {
  return jwt.verify(token, SECRET_KEY_TOKEN, (err, decode) =>
    decode !== undefined ? decode : 0
  );
}

function verifyRefreshToken(token) {
  return jwt.verify(token, SECRET_KEY_REFRESHTOKEN, (err, decode) =>
    decode !== undefined ? null : err
  );
}
...
function isAuthenticated({ user, password }) {
  return (
    userDB.users.findIndex(
      (userArr) => userArr.user === user && userArr.password === password
    ) !== -1
  );
}
...
server.post("/auth/login", (req, res) => {
  ...
  res
    ...
    .json({
      ...
      refreshToken,
      refreshTokenExpireIn: dateRefreshToken,
    });
});
...
server.post("/auth/verifyToken", (req, res) => {
  ...
  const resultOfVerification = verifyTokenForVerificationEndPoint(
...
server.use(/^(?!\/auth).*$/, (req, res, next) => {
  if (req.method !== "GET") {
    ...
    const resultOfVerification = verifyToken(
      req.headers.authorization.split(" ")[1]
    );
    if (!resultOfVerification) {
      next();
...
server.listen(3000, () => {
  ("Server lanciato per il corso di Angular :)");
});
